...
unsigned int POWER_SYSTEM_DATABASE::get_ac_line_capacity() const
{
...
unsigned int POWER_SYSTEM_DATABASE::get_2t_lcc_hvdc_capacity() const
{
...
void POWER_SYSTEM_DATABASE::set_ac_line_capacity(unsigned int n)
{
    Ac_line.reserve(n);
}
...
void POWER_SYSTEM_DATABASE::set_2t_lcc_hvdc_capacity(unsigned int n)
{
...
void POWER_SYSTEM_DATABASE::append_ac_line(AC_LINE& line)
{
    ...
    unsigned int line_count = get_ac_line_count();
...
void POWER_SYSTEM_DATABASE::append_2t_lcc_hvdc(LCC_HVDC2T& hvdc)
{
    ...
    unsigned int hvdc_count = get_2t_lcc_hvdc_count();
...
bool POWER_SYSTEM_DATABASE::is_ac_line_exist(const DEVICE_ID& device_id) const
{
    unsigned int index = get_ac_line_index(device_id);
    if(index!=INDEX_NOT_EXIST) return true;
    else                       return false;
}
...
bool POWER_SYSTEM_DATABASE::is_2t_lcc_hvdc_exist(const DEVICE_ID& device_id) const
{
    unsigned int index = get_2t_lcc_hvdc_index(device_id);
    if(index!=INDEX_NOT_EXIST) return true;
    else                       return false;
}
...
void POWER_SYSTEM_DATABASE::change_bus_number(unsigned int original_bus_number, unsigned int new_bus_number)
{
       ...
       is_bus_in_allowed_range(new_bus_number))
    {
        ...
        for(unsigned int i=0; i!=nline; ++i)
        {
            ...
            unsigned int index = get_ac_line_index(old_did);
        ...
        for(unsigned int i=0; i!=nhvdc; ++i)
        {
            ...
            unsigned int index = get_2t_lcc_hvdc_index(old_did);
...
AC_LINE* POWER_SYSTEM_DATABASE::get_ac_line(const DEVICE_ID & device_id)
{
    unsigned int index = ac_line_index.get_index_of_device(device_id);
    if(index!=INDEX_NOT_EXIST)
...
LCC_HVDC2T* POWER_SYSTEM_DATABASE::get_2t_lcc_hvdc(const DEVICE_ID & device_id)
{
...
vector<AC_LINE*> POWER_SYSTEM_DATABASE::get_ac_lines_connecting_to_bus(const unsigned int bus)
{
    vector<AC_LINE*> device;
    device.reserve(8);

    unsigned int n = get_ac_line_count();
    for(unsigned int i=0; i!=n; ++i)
    {
...
vector<LCC_HVDC2T*> POWER_SYSTEM_DATABASE::get_2t_lcc_hvdcs_connecting_to_bus(const unsigned int bus)
{
    vector<LCC_HVDC2T*> device;
    device.reserve(8);

    unsigned int n = get_2t_lcc_hvdc_count();
    for(unsigned int i=0; i!=n; ++i)
    {
...
vector<DEVICE_ID> POWER_SYSTEM_DATABASE::get_ac_lines_device_id_connecting_to_bus(const unsigned int bus)
{
...
vector<DEVICE_ID> POWER_SYSTEM_DATABASE::get_2t_lcc_hvdcs_device_id_connecting_to_bus(const unsigned int bus)
{
...
vector<AC_LINE*> POWER_SYSTEM_DATABASE::get_ac_lines_in_area(const unsigned int area)
{
    vector<AC_LINE*> device;
    device.reserve(8);
...
vector<LCC_HVDC2T*> POWER_SYSTEM_DATABASE::get_2t_lcc_hvdcs_in_area(const unsigned int area)
{
    vector<LCC_HVDC2T*> device;
    device.reserve(8);
...
vector<DEVICE_ID> POWER_SYSTEM_DATABASE::get_ac_lines_device_id_in_area(const unsigned int area)
{
...
vector<DEVICE_ID> POWER_SYSTEM_DATABASE::get_2t_lcc_hvdcs_device_id_in_area(const unsigned int area)
{
...
vector<AC_LINE*> POWER_SYSTEM_DATABASE::get_ac_lines_in_zone(const unsigned int zone)
{
    vector<AC_LINE*> device;
    device.reserve(8);
...
vector<LCC_HVDC2T*> POWER_SYSTEM_DATABASE::get_2t_lcc_hvdcs_in_zone(const unsigned int zone)
{
    vector<LCC_HVDC2T*> device;
    device.reserve(8);
...
vector<DEVICE_ID> POWER_SYSTEM_DATABASE::get_ac_lines_device_id_in_zone(const unsigned int zone)
{
...
vector<DEVICE_ID> POWER_SYSTEM_DATABASE::get_2t_lcc_hvdcs_device_id_in_zone(const unsigned int zone)
{
...
vector<AC_LINE*> POWER_SYSTEM_DATABASE::get_all_ac_lines()
{
    vector<AC_LINE*> device;
    unsigned int n = get_ac_line_count();
    device.reserve(n);
...
vector<LCC_HVDC2T*> POWER_SYSTEM_DATABASE::get_all_2t_lcc_hvdcs()
{
    vector<LCC_HVDC2T*> device;
    unsigned int n = get_2t_lcc_hvdc_count();
    device.reserve(n);
...
vector<DEVICE_ID> POWER_SYSTEM_DATABASE::get_all_ac_lines_device_id()
{
    vector<DEVICE_ID> dids;
    unsigned int n=get_ac_line_count();
    dids.reserve(n);
...
vector<DEVICE_ID> POWER_SYSTEM_DATABASE::get_all_2t_lcc_hvdcs_device_id()
{
    vector<DEVICE_ID> dids;
    unsigned int n=get_2t_lcc_hvdc_count();
    dids.reserve(n);
    ...
    while(true)
    {
        ...
        for(unsigned int i=0; i!=n; ++i)
        {
            ...
            else
            {
                ...
                {
                    unsigned int ibus = Ac_line[i].get_sending_side_bus();
                    unsigned int jbus = Ac_line[i].get_receiving_side_bus();
...
unsigned int POWER_SYSTEM_DATABASE::get_ac_line_count() const
{
    return Ac_line.size();
}
...
unsigned int POWER_SYSTEM_DATABASE::get_2t_lcc_hvdc_count() const
{
...
unsigned int POWER_SYSTEM_DATABASE::get_ac_line_index(const DEVICE_ID & device_id) const
{
...
unsigned int POWER_SYSTEM_DATABASE::get_2t_lcc_hvdc_index(const DEVICE_ID & device_id) const
{
...
void POWER_SYSTEM_DATABASE::check_all_ac_lines()
{
    unsigned int n = get_ac_line_count();
    for(unsigned int i=0; i!=n; ++i)
        Ac_line[i].check();
...
void POWER_SYSTEM_DATABASE::check_all_2t_lcc_hvdcs()
{
    unsigned int n = get_2t_lcc_hvdc_count();
    for(unsigned int i=0; i!=n; ++i)
        Lcc_hvdc2t[i].check();
...
void POWER_SYSTEM_DATABASE::check_ac_line_related_dynamic_data()
{
    /*vector<AC_LINE*> lines = get_all_ac_lines();
...
void POWER_SYSTEM_DATABASE::check_2t_lcc_hvdc_related_dynamic_data()
{
    vector<LCC_HVDC2T*> hvdcs = get_all_2t_lcc_hvdcs();
...
void POWER_SYSTEM_DATABASE::check_missing_ac_line_related_model()
{
    ostringstream osstream;
...
void POWER_SYSTEM_DATABASE::check_missing_2t_lcc_hvdc_related_model()
{
    ostringstream osstream;
    vector<LCC_HVDC2T*> hvdcs = get_all_2t_lcc_hvdcs();
    unsigned int n = hvdcs.size();
...
void POWER_SYSTEM_DATABASE::clear_ac_line(const DEVICE_ID& device_id)
{
    if(is_ac_line_exist(device_id))
    {
        unsigned int current_index = get_ac_line_index(device_id);
...
void POWER_SYSTEM_DATABASE::clear_ac_lines_connecting_to_bus(const unsigned int bus)
{
    DEVICE_ID device_id;
    TERMINAL terminal;
    device_id.set_device_type(STEPS_AC_LINE);
...
void POWER_SYSTEM_DATABASE::clear_all_ac_lines()
{
    Ac_line.clear();
    ac_line_index.clear();
}
...
void POWER_SYSTEM_DATABASE::clear_2t_lcc_hvdc(const DEVICE_ID& device_id)
{
    if(is_2t_lcc_hvdc_exist(device_id))
    {
        unsigned int current_index = get_2t_lcc_hvdc_index(device_id);
...
void POWER_SYSTEM_DATABASE::clear_2t_lcc_hvdcs_connecting_to_bus(const unsigned int bus)
{
    DEVICE_ID device_id;
    TERMINAL terminal;
    device_id.set_device_type(STEPS_LCC_HVDC2T);
...
void POWER_SYSTEM_DATABASE::clear_all_2t_lcc_hvdcs()
{
    Lcc_hvdc2t.clear();
    hvdc_index.clear();
}
...
void POWER_SYSTEM_DATABASE::check_ac_line_status_for_out_of_service_bus(unsigned int bus)
{
    vector<AC_LINE*> lines = get_ac_lines_connecting_to_bus(bus);
    AC_LINE* line;
    unsigned int nline = lines.size();
...
void POWER_SYSTEM_DATABASE::check_2t_lcc_hvdc_status_for_out_of_service_bus(unsigned int bus)
{
    vector<LCC_HVDC2T*> hvdcs = get_2t_lcc_hvdcs_connecting_to_bus(bus);
    LCC_HVDC2T* hvdc;
    unsigned int nhvdc = hvdcs.size();
...
unsigned int POWER_SYSTEM_DATABASE::get_memory_usage_in_bytes()
{
    ...
    return Bus.capacity()*sizeof(BUS)+
           ...
           ac_line_index.get_map_size()*(sizeof(DEVICE_ID)+sizeof(unsigned int))*2+

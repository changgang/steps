...
void SPARSE_MATRIX_UMFPACK::add_entry(int row, int col, const complex<double>& value)
{
    if(matrix_in_triplet_form())
    {
        ...
        {
            unsigned int r = triplet_row_index[i];
            unsigned int c = triplet_column_index[i];
            if(r==row and c==col)
            {
...
int SPARSE_MATRIX_UMFPACK::get_matrix_row_count()  const
{
    return n_row;
}

int SPARSE_MATRIX_UMFPACK::get_matrix_column_count()  const
{
    return n_column;
}
...
complex<double> SPARSE_MATRIX_UMFPACK::get_complex_entry_value(int index)  const
{
...
vector<unsigned int> SPARSE_MATRIX_UMFPACK::get_reorder_permutation()
{
    if(matrix_in_triplet_form())
        compress_and_merge_duplicate_entries();
...
vector<double>& SPARSE_MATRIX_UMFPACK::solve_Ax_eq_b(vector<double>& b)
{
    ...
    for(unsigned int i=0; i<n_row; ++i) B[i] = b[i];
    ...
    for(unsigned int i=0; i<n_row; ++i) b[i] = x[i];
    free(x);
    free(B);
    return b;
}

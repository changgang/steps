...
class STEPS():
    ...
    def get_package_float_data(self,dataname):
        return self.__libsteps.api_get_package_float_data(dataname)        

    def get_allowed_maximum_bus_number(self):
        return int(self.__libsteps.api_get_allowed_maximum_bus_number())
    ...
    def get_bus_capacity(self):
        return int(self.__libsteps.api_get_device_capacity("BUS"))
    def get_generator_capacity(self):
        return int(self.__libsteps.api_get_device_capacity("Generator"))
    def get_wt_generator_capacity(self):
        return int(self.__libsteps.api_get_device_capacity("WT Generator"))
    def get_pv_unit_capacity(self):
        return int(self.__libsteps.api_get_device_capacity("PV Unit"))
    def get_load_capacity(self):
        return int(self.__libsteps.api_get_device_capacity("Load"))
    def get_fixed_shunt_capacity(self):
        return int(self.__libsteps.api_get_device_capacity("Fixed Shunt"))
    def get_line_capacity(self):
        return int(self.__libsteps.api_get_device_capacity("Line"))
    def get_transformer_capacity(self):
        return int(self.__libsteps.api_get_device_capacity("Transformer"))
    def get_hvdc_capacity(self):
        return int(self.__libsteps.api_get_device_capacity("HVDC"))
    def get_equivalent_device_capacity(self):
        return int(self.__libsteps.api_get_device_capacity("Equivalent Device"))
    def get_energy_storage_capacity(self):
        return int(self.__libsteps.api_get_device_capacity("Energy Storage"))
    def get_area_capacity(self):
        return int(self.__libsteps.api_get_area_capacity())
    def get_zone_capacity(self):
        return int(self.__libsteps.api_get_zone_capacity())
    def get_owner_capacity(self):
        return int(self.__libsteps.api_get_owner_capacity())
    ...
    def set_pv_unit_capacity(self, capacity):
        return self.__libsteps.api_set_device_capacity("PV Unit", capacity)
    ...
    def set_energy_storage_capacity(self, capacity):
        return self.__libsteps.api_set_device_capacity("Energy Storage", capacity)
    ...
    def add_pv_unit(self, unit):
        ...
        self.__libsteps.api_add_pv_unit(bus, ickt)
        return
    ...
    def add_energy_storage(self, storage):
        ...
        self.__libsteps.api_add_energy_storage(bus, ickt)
        return
    ...
    def get_pv_unit_count(self):
        return self.__libsteps.api_get_device_count("PV Unit")
    ...
    def get_energy_storage_count(self):
        return self.__libsteps.api_get_device_count("Energy Storage")
    ...
    def is_bus_exist(self, bus):
        return self.__libsteps.api_is_bus_exist(bus)

    def is_generator_exist(self, generator):
        bus, ickt = self.__extract_single_bus_device_id(generator)
        return self.__libsteps.api_is_generator_exist(bus, ickt)

    def is_wt_generator_exist(self, generator):
        bus, ickt = self.__extract_single_bus_device_id(generator)
        return self.__libsteps.api_is_wt_generator_exist(bus, ickt)

    def is_pv_unit_exist(self, pv_unit):
        bus, ickt = self.__extract_single_bus_device_id(pv_unit)
        return self.__libsteps.api_is_pv_unit_exist(bus, ickt)

    def is_load_exist(self, load):
        bus, ickt = self.__extract_single_bus_device_id(load)
        return self.__libsteps.api_is_load_exist(bus, ickt)

    def is_fixed_shunt_exist(self, shunt):
        bus, ickt = self.__extract_single_bus_device_id(shunt)
        return self.__libsteps.api_is_fixed_shunt_exist(bus, ickt)

    def is_line_exist(self, line):
        ibus, jbus, ickt = self.__extract_double_bus_device_id(line)
        return self.__libsteps.api_is_line_exist(ibus, jbus, ickt)

    def is_transformer_exist(self, transformer):
        ibus, jbus, kbus, ickt = self.__extract_triple_bus_device_id(transformer)
        return self.__libsteps.api_is_transformer_exist(ibus, jbus, kbus, ickt)

    def is_hvdc_exist(self, hvdc):
        ibus, jbus, ickt = self.__extract_double_bus_device_id(hvdc)
        return self.__libsteps.api_is_hvdc_exist(ibus, jbus, ickt)

    def is_equivalent_device_exist(self, equivalent_device):
        bus = equivalent_device[0]
        ickt = equivalent_device[1]
        return self.__libsteps.api_is_equivalent_device_exist(bus, ickt)

    def is_energy_storage_exist(self, energy_storage):
        bus = energy_storage[0]
        ickt = energy_storage[1]
        return self.__libsteps.api_is_energy_storage_exist(bus, ickt)
    ...
    def get_pv_units_at_bus(self, bus):
        self.__libsteps.api_initialize_device_search("PV UNIT", bus)
        pv_units = []
        while True:
            bus = self.__libsteps.api_get_current_device_bus_number("PV UNIT", "")
            if bus==0:
                break
            id = self.__libsteps.api_get_current_device_identifier("PV UNIT")
            pv_units.append((bus, id))
            self.__libsteps.api_goto_next_device("PV UNIT")
    ...
    def get_energy_storages_at_bus(self, bus):
        self.__libsteps.api_initialize_device_search("ENERGY STORAGE", bus)
        energy_storages = []
        while True:
            bus = self.__libsteps.api_get_current_device_bus_number("ENERGY STORAGE", "")
            if bus==0:
                break
            id = self.__libsteps.api_get_current_device_identifier("ENERGY STORAGE")
            energy_storages.append((bus, id))
            self.__libsteps.api_goto_next_device("ENERGY STORAGE")
    ...
    def get_bus_data(self, bus, par_type, par_name):
        ...
        if par_type in ['I', 'INT', 'INTEGER']:
            return int(self.__libsteps.api_get_bus_integer_data(bus, par_name))
    ...
    def __get_source_data(self, source, par_type, par_name):
        ...
        if par_type in ['I', 'INT', 'INTEGER']:
            return int(self.__libsteps.api_get_source_integer_data(bus, ickt, par_name))
        if par_type in ['F', 'D', 'FLOAT', 'DOUBLE']:
            return self.__libsteps.api_get_source_float_data(bus, ickt, par_name)
        if par_type in ['B', 'BOOL', 'BOOLEAN']:
            return self.__libsteps.api_get_source_boolean_data(bus, ickt, par_name)
        if par_type in ['S', 'STRING']:
            return self.__libsteps.api_get_source_string_data(bus, ickt, par_name)
        return None
    ...
    def get_load_data(self, load, par_type, par_name):
        ...
        bus, ickt = self.__extract_single_bus_device_id(load)
        if par_type in ['I', 'INT', 'INTEGER']:
            return int(self.__libsteps.api_get_load_integer_data(bus, ickt, par_name))
        if par_type in ['F', 'D', 'FLOAT', 'DOUBLE']:
            return self.__libsteps.api_get_load_float_data(bus, ickt, par_name)
        if par_type in ['B', 'BOOL', 'BOOLEAN']:
            return self.__libsteps.api_get_load_boolean_data(bus, ickt, par_name)
        if par_type in ['S', 'STRING']:
            return self.__libsteps.api_get_load_string_data(bus, ickt, par_name)
        return None
    ...
    def get_fixed_shunt_data(self, fixed_shunt, par_type, par_name):
        ...
        if par_type in ['I', 'INT', 'INTEGER']:
            return int(self.__libsteps.api_get_fixed_shunt_integer_data(bus, ickt, par_name))
        if par_type in ['F', 'D', 'FLOAT', 'DOUBLE']:
            return self.__libsteps.api_get_fixed_shunt_float_data(bus, ickt, par_name)
        if par_type in ['B', 'BOOL', 'BOOLEAN']:
            return self.__libsteps.api_get_fixed_shunt_boolean_data(bus, ickt, par_name)
        if par_type in ['S', 'STRING']:
            return self.__libsteps.api_get_fixed_shunt_string_data(bus, ickt, par_name)
        return None
    ...
    def get_equivalent_device_data(self, equivalent_device, par_type, par_name):
        ...
        if par_type in ['I', 'INT', 'INTEGER']:
            return int(self.__libsteps.api_get_equivalent_device_integer_data(bus, ickt, par_name))
        if par_type in ['F', 'D', 'FLOAT', 'DOUBLE']:
            return self.__libsteps.api_get_equivalent_device_float_data(bus, ickt, par_name)
        if par_type in ['B', 'BOOL', 'BOOLEAN']:
            return self.__libsteps.api_get_equivalent_device_boolean_data(bus, ickt, par_name)
        if par_type in ['S', 'STRING']:
            return self.__libsteps.api_get_equivalent_device_string_data(bus, ickt, par_name)
        return None
    ...
    def get_line_data(self, line, par_type, par_name):
        ...
        ibus, jbus, ickt = self.__extract_double_bus_device_id(line)
        if par_type in ['I', 'INT', 'INTEGER']:
            return int(self.__libsteps.api_get_line_integer_data(ibus, jbus, ickt, par_name))
        if par_type in ['F', 'D', 'FLOAT', 'DOUBLE']:
            return self.__libsteps.api_get_line_float_data(ibus, jbus, ickt, par_name)
        if par_type in ['B', 'BOOL', 'BOOLEAN']:
            return self.__libsteps.api_get_line_boolean_data(ibus, jbus, ickt, par_name)
        if par_type in ['S', 'STRING']:
            return self.__libsteps.api_get_line_string_data(ibus, jbus, ickt, par_name)
        return None
    ...
    def get_transformer_data(self, transformer, par_type, side, par_name):
        ...
        ibus, jbus, kbus, ickt = self.__extract_triple_bus_device_id(transformer)
        if par_type in ['I', 'INT', 'INTEGER']:
            return int(self.__libsteps.api_get_transformer_integer_data(ibus, jbus, kbus, ickt, side, par_name))
        if par_type in ['F', 'D', 'FLOAT', 'DOUBLE']:
            return self.__libsteps.api_get_transformer_float_data(ibus, jbus, kbus, ickt, side, par_name)
        if par_type in ['B', 'BOOL', 'BOOLEAN']:
            return self.__libsteps.api_get_transformer_boolean_data(ibus, jbus, kbus, ickt, side, par_name)
        if par_type in ['S', 'STRING']:
            return self.__libsteps.api_get_transformer_string_data(ibus, jbus, kbus, ickt, side, par_name)
        return None
    ...
    def get_hvdc_data(self, hvdc, par_type, side, par_name):
        ...
        ibus, jbus, ickt = self.__extract_double_bus_device_id(hvdc)
        if par_type in ['I', 'INT', 'INTEGER']:
            return int(self.__libsteps.api_get_hvdc_integer_data(ibus, jbus, ickt, side, par_name))
        if par_type in ['F', 'D', 'FLOAT', 'DOUBLE']:
            return self.__libsteps.api_get_hvdc_float_data(ibus, jbus, ickt, side, par_name)
        if par_type in ['B', 'BOOL', 'BOOLEAN']:
            return self.__libsteps.api_get_hvdc_boolean_data(ibus, jbus, ickt, side, par_name)
        if par_type in ['S', 'STRING']:
            return self.__libsteps.api_get_hvdc_string_data(ibus, jbus, ickt, side, par_name)
        return None
    ...
    def get_area_data(self, area, par_type, par_name):
        ...
        if par_type not in ['I', 'INT', 'INTEGER', 'F', 'D', 'FLOAT', 'DOUBLE', 'B', 'BOOL', 'BOOLEAN', 'S', 'STRING']:
            return None
        if par_type in ['I', 'INT', 'INTEGER']:
            return int(self.__libsteps.api_get_area_integer_data(area, par_name))
        if par_type in ['F', 'D', 'FLOAT', 'DOUBLE']:
    ...
    def get_zone_data(self, zone, par_type, par_name):
        ...
        if par_type not in ['I', 'INT', 'INTEGER', 'F', 'D', 'FLOAT', 'DOUBLE', 'B', 'BOOL', 'BOOLEAN', 'S', 'STRING']:
            return None
        if par_type in ['I', 'INT', 'INTEGER']:
            return int(self.__libsteps.api_get_zone_integer_data(zone, par_name))
        if par_type in ['F', 'D', 'FLOAT', 'DOUBLE']:
    ...
    def get_owner_data(self, owner, par_type, par_name):
        ...
        if par_type not in ['I', 'INT', 'INTEGER', 'F', 'D', 'FLOAT', 'DOUBLE', 'B', 'BOOL', 'BOOLEAN', 'S', 'STRING']:
            return None
        if par_type in ['I', 'INT', 'INTEGER']:
            return int(self.__libsteps.api_get_owner_integer_data(owner, par_name))
        if par_type in ['F', 'D', 'FLOAT', 'DOUBLE']:
    ...
    def __set_source_data(self, source, par_type, par_name, value):
        ...
        bus, ickt = self.__extract_single_bus_device_id(line)
        if par_type in ['I', 'INT', 'INTEGER']:
            return self.__libsteps.api_set_source_integer_data(bus, ickt, par_name, value)
        if par_type in ['F', 'D', 'FLOAT', 'DOUBLE']:
            return self.__libsteps.api_set_source_float_data(bus, ickt, par_name, value)
        if par_type in ['B', 'BOOL', 'BOOLEAN']:
            return self.__libsteps.api_set_source_boolean_data(bus, ickt, par_name, value)
        if par_type in ['S', 'STRING']:
            return self.__libsteps.api_set_source_string_data(bus, ickt, par_name, value)
        return
    ...
    def set_load_data(self, load, par_type, par_name, value):
        ...
        bus, ickt = self.__extract_single_bus_device_id(load)
        if par_type in ['I', 'INT', 'INTEGER']:
            return self.__libsteps.api_set_load_integer_data(bus, ickt, par_name, value)
        if par_type in ['F', 'D', 'FLOAT', 'DOUBLE']:
            return self.__libsteps.api_set_load_float_data(bus, ickt, par_name, value)
        if par_type in ['B', 'BOOL', 'BOOLEAN']:
            return self.__libsteps.api_set_load_boolean_data(bus, ickt, par_name, value)
        if par_type in ['S', 'STRING']:
            return self.__libsteps.api_set_load_string_data(bus, ickt, par_name, value)
        return
    ...
    def set_fixed_shunt_data(self, fixed_shunt, par_type, par_name, value):
        ...
        bus, ickt = self.__extract_single_bus_device_id(fixed_shunt)
        if par_type in ['I', 'INT', 'INTEGER']:
            return self.__libsteps.api_set_fixed_shunt_integer_data(bus, ickt, par_name, value)
        if par_type in ['F', 'D', 'FLOAT', 'DOUBLE']:
            return self.__libsteps.api_set_fixed_shunt_float_data(bus, ickt, par_name, value)
        if par_type in ['B', 'BOOL', 'BOOLEAN']:
            return self.__libsteps.api_set_fixed_shunt_boolean_data(bus, ickt, par_name, value)
        if par_type in ['S', 'STRING']:
            return self.__libsteps.api_set_fixed_shunt_string_data(bus, ickt, par_name, value)
        return
    ...
    def set_equivalent_device_data(self, equivalent_device, par_type, par_name, value):
        ...
        bus, ickt = self.__extract_single_bus_device_id(equivalent_device)
        if par_type in ['I', 'INT', 'INTEGER']:
            return self.__libsteps.api_set_equivalent_device_integer_data(bus, ickt, par_name, value)
        if par_type in ['F', 'D', 'FLOAT', 'DOUBLE']:
            return self.__libsteps.api_set_equivalent_device_float_data(bus, ickt, par_name, value)
        if par_type in ['B', 'BOOL', 'BOOLEAN']:
            return self.__libsteps.api_set_equivalent_device_boolean_data(bus, ickt, par_name, value)
        if par_type in ['S', 'STRING']:
            return self.__libsteps.api_set_equivalent_device_string_data(bus, ickt, par_name, value)
        return
    ...
    def set_line_data(self, line, par_type, par_name, value):
        ...
        ibus, jbus, ickt = self.__extract_double_bus_device_id(line)
        if par_type in ['I', 'INT', 'INTEGER']:
            return self.__libsteps.api_set_line_integer_data(ibus, jbus, ickt, par_name, value)
        if par_type in ['F', 'D', 'FLOAT', 'DOUBLE']:
            return self.__libsteps.api_set_line_float_data(ibus, jbus, ickt, par_name, value)
        if par_type in ['B', 'BOOL', 'BOOLEAN']:
            return self.__libsteps.api_set_line_boolean_data(ibus, jbus, ickt, par_name, value)
        if par_type in ['S', 'STRING']:
            return self.__libsteps.api_set_line_string_data(ibus, jbus, ickt, par_name, value)
        return
    ...
    def set_transformer_data(self, transformer, par_type, par_name, value):
        ...
        if par_type in ['I', 'INT', 'INTEGER']:
            return self.__libsteps.api_set_transformer_integer_data(ibus, jbus, kbus, ickt, par_name, value)
        if par_type in ['F', 'D', 'FLOAT', 'DOUBLE']:
            return self.__libsteps.api_set_transformer_float_data(ibus, jbus, kbus, ickt, par_name, value)
        if par_type in ['B', 'BOOL', 'BOOLEAN']:
            return self.__libsteps.api_set_transformer_boolean_data(ibus, jbus, kbus, ickt, par_name, value)
        if par_type in ['S', 'STRING']:
            return self.__libsteps.api_set_transformer_string_data(ibus, jbus, kbus, ickt, par_name, value)
        return
    ...
    def set_hvdc_data(self, hvdc, par_type, par_name, value):
        ...
        ibus, jbus, ickt = self.__extract_double_bus_device_id(hvdc)
        if par_type in ['I', 'INT', 'INTEGER']:
            return self.__libsteps.api_set_hvdc_integer_data(ibus, jbus, ickt, par_name, value)
        if par_type in ['F', 'D', 'FLOAT', 'DOUBLE']:
            return self.__libsteps.api_set_hvdc_float_data(ibus, jbus, ickt, par_name, value)
        if par_type in ['B', 'BOOL', 'BOOLEAN']:
            return self.__libsteps.api_set_hvdc_boolean_data(ibus, jbus, ickt, par_name, value)
        if par_type in ['S', 'STRING']:
            return self.__libsteps.api_set_hvdc_string_data(ibus, jbus, ickt, par_name, value)
        return
    ...
    def get_pv_unit_related_model_name(self, pv_unit, model_type):
        return self.__libsteps.api_get_pv_unit_related_model_name(pv_unit[0], pv_unit[1], model_type)
    ...
    def get_pv_unit_related_model_data(self, pv_unit, model_type, par_type, par_name):
        ...
        if par_type in ['I', 'INT', 'INTEGER']:
            return 0
        if par_type in ['F', 'D', 'FLOAT', 'DOUBLE']:
            return self.__libsteps.api_get_pv_unit_related_model_float_parameter(pv_unit[0], pv_unit[1], model_type, par_name)
        if par_type in ['B', 'BOOL', 'BOOLEAN']:
            return False
        return None
    ...
    def set_pv_unit_related_model_data(self, pv_unit, model_type, par_type, par_name, value):
        ...
        if par_type in ['I', 'INT', 'INTEGER']:
            return
        if par_type in ['F', 'D', 'FLOAT', 'DOUBLE']:
            return self.__libsteps.api_set_pv_unit_related_model_float_parameter(pv_unit[0], pv_unit[1], model_type, par_name, value)
        if par_type in ['B', 'BOOL', 'BOOLEAN']:
            return
        return
    ...
    def get_powerflow_solver_parameter(self, par_type, par_name):
        ...
        if par_type not in ['I', 'INT', 'INTEGER', 'F', 'D', 'FLOAT', 'DOUBLE', 'B', 'BOOL', 'BOOLEAN']:
            return None
        if par_type in ['I', 'INT', 'INTEGER']:
            return int(self.__libsteps.api_get_powerflow_solver_integer_parameter(par_name))
        if par_type in ['F', 'D', 'FLOAT', 'DOUBLE']:
    ...
    def get_dynamic_simulator_parameter(self, par_type, par_name):
        ...
        if par_type not in ['I', 'INT', 'INTEGER', 'F', 'D', 'FLOAT', 'DOUBLE', 'B', 'BOOL', 'BOOLEAN', 'S', 'STRING']:
            return None
        if par_type in ['I', 'INT', 'INTEGER']:
            return int(self.__libsteps.api_get_dynamic_simulator_integer_parameter(par_name))
        if par_type in ['F', 'D', 'FLOAT', 'DOUBLE']:
    ...
    def prepare_generator_meter(self, generator, meter_type):
        bus, ickt = self.__extract_single_bus_device_id(generator)
        self.__libsteps.api_prepare_generator_related_meter(bus, ickt, meter_type)
        return

    def prepare_wt_generator_meter(self, generator, meter_type):
        bus, ickt = self.__extract_single_bus_device_id(generator)
        self.__libsteps.api_prepare_wt_generator_related_meter(bus, ickt, meter_type)
        return

    def prepare_load_meter(self, load, meter_type):
        bus, ickt = self.__extract_single_bus_device_id(load)
        self.__libsteps.api_prepare_load_related_meter(bus, ickt, meter_type)
        return

    def prepare_line_meter(self, line, meter_type, side):
        ibus, jbus, ickt = self.__extract_double_bus_device_id(line)
        self.__libsteps.api_prepare_line_related_meter(ibus, jbus, ickt, meter_type, side)
        return

    def prepare_hvdc_meter(self, hvdc, meter_type, side):
        ibus, jbus, ickt = self.__extract_double_bus_device_id(hvdc)
        self.__libsteps.api_prepare_hvdc_related_meter(ibus, jbus, ickt, meter_type, side)
        return

    def prepare_equivalent_device_meter(self, edevice, meter_type):
        bus, ickt = self.__extract_single_bus_device_id(equivalent_device)
        self.__libsteps.api_prepare_equivalent_device_related_meter(bus, ickt, meter_type)
        return
    ...
    def trip_transformer(self, transformer):
        ibus, jbus, kbus, ickt = self.__extract_triple_bus_device_id(transformer)
        self.__libsteps.api_trip_transformer(ibus, jbus, kbus, ickt)
        return

    def trip_transformer_breaker(self, transformer):
        ibus, jbus, kbus, ickt = self.__extract_triple_bus_device_id(transformer)
        self.__libsteps.api_trip_transformer_breaker(ibus, jbus, kbus, ickt)
        return
        

    def close_transformer(self, transformer):
        ibus, jbus, kbus, ickt = self.__extract_triple_bus_device_id(transformer)
        self.__libsteps.api_close_transformer(ibus, jbus, kbus, ickt)
        return

    def close_transformer_breaker(self, transformer):
        ibus, jbus, kbus, ickt = self.__extract_triple_bus_device_id(transformer)
        self.__libsteps.api_close_transformer_breaker(ibus, jbus, kbus, ickt)
        return
